...
export default function ComponentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const relRes = await fetch(`/api/components?category=${encodeURIComponent(data.category)}&limit=5`);
        const relData = await relRes.json();
        const filtered = (relData.components || []).filter((c: any) => c.id !== data.id).slice(0, 4);
        setRelated(filtered);
      } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Specifications and Pinout Section */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {related.map((comp) => (
              ...
              >
                ...
                <h3 className="font-semibold text-foreground group-hover:text-cyan-400">{comp.name}</h3>
